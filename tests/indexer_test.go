...
func TestHashMapIndexer_Add(t *testing.T) {
	...
	err := indexer.Add(1, 10)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}

	err = indexer.Add(1, 20)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}

	err = indexer.Add(1, 10) // should fail
	if err == nil {
